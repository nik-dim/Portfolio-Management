...
\begin{frame}
    \frametitle{Optimization yields...}
    \framesubtitle{Optimal weights}
    \begin{equation*}
        ...
        20.0\\ \tikz[baseline]{
            \node[fill=blue!20,anchor=base] (t100)
            {\( 0.0 \)};}\\\tikz[baseline]{
                \node[fill=blue!20,anchor=base] (t200)
                {\( 0.0 \)};}\\10.0
        \end{bmatrix}\%
    \end{equation*}
    
    \begin{itemize}
    	\item Assets with zero weight allocation can either be excluded or substituted by other assets of the same class
    \end{itemize}
    % \colrow[blue ,opacity=.5]{num0}{num1}
    % \colrow[blue ,opacity=.5]{num2}{num3}
\end{frame}
...
\begin{frame}
    \frametitle{Optimal Overall Portfolio}
    \begin{alertblock}{Which portfolio is the overall optimal?}
         \begin{itemize}
             \item i.e. which portfolio along the CAL is the best \pause \textbf{for us}?
         \end{itemize}
    \end{alertblock}
    \pause
...
% Therefore, to maximize the utility w.r.t the allocation \( z \) we set the the optimal \(z*\) that maximizes utility is:


% After calculations, we derive that \(z^{*}=0.76\) and \(U_{max}=13.87\). By finding different levels of risk an return that yield the same value of maximum utility, we can graph the indifference curve for \(U_{max}=13.87\) and \(A=1.5\). As illustrated in Figure \ref{fig:final_graph}, the optimal overall portfolio occurs at the point where the indifference curve is tangent to the CAL of the optimal risky portfolio.
...
\begin{frame}
    \frametitle{Past Performance}
    \framesubtitle{(1/13/2015 - 1/13/2020)}
    \begin{center}
        \begin{tabular}{l|cc}
            \toprule
            {}   & Optimal Overall Portfolio & S\&P 500 \\
            \midrule
            Return (\%) & 25.4769 & 11.3118\\
            Risk (\%) & 12.4384 & 13.4138\\
            Sharpe Ratio & 1.8658 & 0.6741\\   
            Beta (\( \beta \)) & 0.6911 & 1.0 \\
            Alpha (\( \alpha \)) (\%) & 16.9584 & 0.0\\
            \bottomrule
        \end{tabular}
    \end{center}
...
% \paragraph*{Holding Period Returns (1/14/2015 - Now)} It is interesting to additionally calculate the Holding Period Return (HPR) of the overall portfolio and compare it to the market index, as the world economy has experienced a rocky ride in the last quarter, due to reasons partly presented in Section \ref{events}. The formula is presented below:
...
% The overall portfolio has managed to moderate its losses, due to the inclusion of both bonds and commodities that offer risk diversification. Therefore the overall decline observed for the market, represented by the \texttt{S\&P 500} index, was partially mitigated to our overall portfolio.
