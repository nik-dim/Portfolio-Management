...
As an initial approach, available capital is allocated uniformly between all assets in the class.In other words, let \( x\% \) is allocated to a class of \( n \) assets (i.e. stocks, bonds or commodities). Then, each security of this class receives a \( \frac{x\%}{n} \) allocation. Hence, the weights associated with the securities are:
...
This naive allocation yiels the following results:
\begin{todo}{naive}{todo:naive}
    \begin{center}
        \begin{tabular}{l|r}
        \toprule
            & Results \\\hline
