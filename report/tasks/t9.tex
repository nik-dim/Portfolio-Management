...
After calculations, we derive that \(z^{*}=0.95\) and \(U_{max}=13.87\). By finding different levels of risk an return that yield the same value of maximum utility, we can graph the indifference curve for \(U_{max}=13.87\) and \(A=1.5\). As illustrated in Figure \ref{fig:final_graph}, the optimal overall portfolio occurs at the point where the indifference curve is tangent to the CAL of the optimal risky portfolio.
