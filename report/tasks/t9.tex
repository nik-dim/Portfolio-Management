...
\begin{equation}
\label{eq:utility}
\text{Utility} = r_{f} + z \cdot (r_{\text{opt}} - r_{f}) - 0.05 \cdot A \cdot \sigma_{\text{opt}}^{2} \cdot z^{2}
\end{equation}

Therefore the optimal \(z\) that maximizes utility is:

\begin{equation}
\label{eq:z_optimal}
z^{*} = \cfrac{r_{\text{opt}} - r_f}{0.1 \cdot A \cdot \sigma_{\text{opt}}^2}
\end{equation}
