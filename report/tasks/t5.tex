...
\begin{itemize}
	\item From daily prices, calculate the daily returns \( r_i = \frac{d_i-d_{i-1}}{d_{i-1}}\)
	\item Calculate the mean \( \bar{r}_{daily} \) and standard deviation \( \sigma_{daily} \).
	\item The annualized return is given from the following equation:
	\begin{equation}
	\label{eq:annualized_return}
	\text{Annualized Return (\%)} =  [(1+\bar{r}_{daily})^{252}-1] \cdot 100
	\end{equation}
	\item The annualized standard deviation is given from the following equation:
	\begin{equation}
	\label{eq:annualized_std}
	\text{Annualized Standard Deviation (\%)} =  \sigma_{daily} \cdot \sqrt{252} \cdot 100
	\end{equation}
\end{itemize}
