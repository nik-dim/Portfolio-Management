...
The correlation matrix is also shown in Figure \ref{fig:correlation matrix} and in Figure \ref{fig:pairplot} another visualization of the effect of the various values of correlation coefficients is presented:
\begin{figure}[H]
    \centering
...
\end{itemize}
To calculate the Sharpe Ratio we use "the trailing \textit{five}-year period by dividing a fund's annualized excess returns over the risk-free rate by its annualized standard deviation"\footnote{\href{https://www.morningstar.com/InvGlossary/sharpe_ratio.aspx?fbclid=IwAR3SJ8lYFFHd8nhWYhOoMcq8v0hOhMuvDXnA9G58gbYwXixBjmb7pa-c5vs}{link}}.

Using the historical data for each fund and the analysis presented above, we are able to calculate \( \alpha, \beta, R^2 \) and the Sparpe Ratio using \texttt{Python}. The results are presented below:
...
Historical data of a 5-year time frame with daily intervals were used for the above calculations.
